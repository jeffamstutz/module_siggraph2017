...
#define INVERT_RMB
    ...
    void ImGui3DWidget::create(const char *title, bool fullScreen)
    {
      ...
      auto defaultSizeFromEnv =
        utility::getEnvVar<std::string>("OSPRAY_APPS_DEFAULT_WINDOW_SIZE");
    ...
    void run()
    {
      ...
      while (!glfwWindowShouldClose(window)) {
        ...
        if (!ImGui::Begin("Example: Fixed Overlay", &open,
          ...
          ImGui::PushStyleColor(ImGuiCol_Text, ImColor(.2,.2,.2,1.f));
          ...
          ImGui::PushStyleColor(ImGuiCol_Text, ImColor(.94,.94,.94,1.f));
    ...
    void init(int32_t *ac, const char **av)
    {
      for(int i = 1; i < *ac;i++) {
        ...
        if (arg == "-win") {
          ...
          auto& handedness = viewPortFromCmdLine->handedness;
          ...
          if (!viewPortFromCmdLine)
            viewPortFromCmdLine = new ImGui3DWidget::ViewPort;
          std::string token = av[i+1];
          if (token == "left")
            viewPortFromCmdLine->handedness = 0;
          else if (token == "right")
            viewPortFromCmdLine->handedness = 1;
          else
            throw std::runtime_error("Unrecognized token:  \"" + token + '\"');
          assert(i+1 < *ac);
          removeArgs(*ac,(char **&)av,i,2); --i;
          ...
          if (!viewPortFromCmdLine)
            viewPortFromCmdLine = new ImGui3DWidget::ViewPort;
          viewPortFromCmdLine->shutter.x = atof(av[i+1]);
          viewPortFromCmdLine->shutter.y = atof(av[i+2]);
          assert(i+2 < *ac);
          removeArgs(*ac,(char **&)av,i,3); --i;
          continue;
        }
    ...
    void ImGui3DWidget::keypress(char key)
    {
      ...
      case 'm': {
        ...
        if (motion_blur) viewPort.shutter = vec2f(0.0f,1.0f);
        else             viewPort.shutter = vec2f(0.0f,0.0f);
        viewPort.modified = true;
        break;
      }
      case 27 /*ESC*/:
