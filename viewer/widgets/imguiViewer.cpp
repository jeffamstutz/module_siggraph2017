...
namespace ospray {
  ...
  ImGuiViewer::ImGuiViewer(const std::deque<box3f> &worldBounds,
    ...
    : ImGuiViewer(worldBounds, model, renderer,
                  cpp::Renderer(), cpp::FrameBuffer(), camera, initialManipulator)
  {
  }
  ...
  ImGuiViewer::ImGuiViewer(const std::deque<box3f> &worldBounds,
                           ...
                           cpp::Camera camera,
                           ManipulatorMode initialManipulator)
    : ImGui3DWidget(ImGui3DWidget::FRAMEBUFFER_NONE, initialManipulator),
      sceneModels(model),
  ...
  void ImGuiViewer::display()
  {
    ...
    if (viewPort.modified) {
      ...
      camera.set("dir", dir);
      camera.set("focusDistance", length(dir));
  ...
  void ImGuiViewer::buildGui()
  {
    ...
    if (ImGui::CollapsingHeader("FPS Statistics", "FPS Statistics",
                                true, true)) {
      ImGui::NewLine();
      ImGui::Text("OSPRay render rate: %.2f fps", lastFrameFPS);
      ImGui::Text("  GUI display rate: %.2f fps", ImGui::GetIO().Framerate);
      ImGui::NewLine();
    }
