...
namespace commandline {
  ...
  bool DefaultRendererParser::parse(int ac, const char **&av)
  {
    for (int i = 1; i < ac; i++) {
      ...
      if (arg == "--renderer" || arg == "-r") {
        ...
        maxDepth = atoi(av[++i]);
      } else if (arg == "--max-radiance") {
        maxRadiance = atof(av[++i]);
      } else if (arg == "--tone-gain") {
        toneGain = atof(av[++i]);
      } else if (arg == "--tone-burn") {
        toneBurn = atof(av[++i]);
      }
    }
  ...
  void DefaultRendererParser::finalize()
  {
    ...
    parsedRenderer.set("maxRadiance", maxRadiance);

    parsedRenderer.set("toneGain", toneGain);
    parsedRenderer.set("toneBurn", toneBurn);
