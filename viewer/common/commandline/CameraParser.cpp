...
bool DefaultCameraParser::parse(int ac, const char **&av)
{
  for (int i = 1; i < ac; i++) {
    ...
    if (arg == "--camera" || arg == "-c") {
      ...
      fovy = atof(av[++i]);
    } else if (arg == "--lens-radius") {
      lensRadius = atof(av[++i]);
    } else if (arg == "--focus-dist") {
      focusDist = atof(av[++i]);
    } else if (arg == "--handedness") {
    ...
    else if (arg == "--shutter" || arg == "-shutter") {
      shutter.x = atof(av[++i]);
      shutter.y = atof(av[++i]);
    }
  }
...
void DefaultCameraParser::finalize()
{
  ...
  parsedCamera.set("apertureRadius", lensRadius);
  parsedCamera.set("focusDistance", focusDist);
  parsedCamera.set("handedness", handedness);
  parsedCamera.set("shutter",shutter);
  parsedCamera.commit();
}
